...
export default function Post (props) {
  return (
    <div key={props.post.fields.slug} style={{display: 'flex', flexDirection: 'column'}}>
      {props.post.frontmatter.thumbnail && (
        <Thumbnail sizes={props.post.frontmatter.thumbnail.childImageSharp.sizes}/>
      )}
      ...
      <p style={{flex: 1 }}dangerouslySetInnerHTML={{ __html: props.post.excerpt }} />

      {props.post.frontmatter.tags.length && (
        <Tags>
          {props.post.frontmatter.tags.map(tag => 
            <Tag to={`/tags/${kebabCase(tag)}`} key={tag}>{tag}</Tag>)}
        </Tags>
      )}
    </div>
  );
